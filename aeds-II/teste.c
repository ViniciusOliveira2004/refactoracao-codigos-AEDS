...
void Pesquisa(Registro *x, TipoApontador *p)
{ if (*p == NULL) 
...
void Insere(Registro x, TipoApontador *p)
{ if (*p == NULL) 
...
void removePredecessor(TipoApontador q, TipoApontador *r)
{ if ((*r)->Dir != NULL) 
  { removePredecessor(q, &(*r)->Dir);
    return;
  }
  q->Reg = (*r)->Reg;
  q = *r; 
...
void Retira(Registro x, TipoApontador *p)
{  TipoApontador Aux;
  if (*p == NULL) 
...
int main(int argc, char *argv[])
{
  ...
  Registro x; TipoChave vetor[MAX];
  int i, j, k, n;
